...
GAME_NAME_SHORT = "wwiu2"

MULTIPLIER = float(raw_input('Enter a resolution multiplier (1): ') or 1)
MULT = lambda x: x.__class__(x * MULTIPLIER)

SCREEN_WIDTH = MULT(520)
SCREEN_HEIGHT = MULT(520)

PLAY_AREA_WIDTH = MULT(520)
PLAY_AREA_HEIGHT = MULT(520)
...
#In pixels per frame or pixels per frame^2
PLAYER_MAX_SPEED = MULT(6.0)
PLAYER_MAX_ACC = MULT(4.0)
...
SPIKES_VER_OFFSET = MULT(4)

#In frames
SPIDER_FIRE_DELAY = 30

SPIDER_TOO_WIDE = MULT(7)
SPIDER_PROJECTILE_SPEED = MULT(5.0)
SPIDER_DAMAGE = 5
